#共通パーティクル＆サウンド
    particle explosion ^ ^1 ^1.25
    execute as @s[tag=attacked_6] at @s run particle enchanted_hit ^ ^1 ^1.5 0.2 0.2 0.2 0.5 10
    playsound entity.player.attack.sweep voice @a ~ ~ ~ 1 0.75 0
    

##攻撃判定
    execute anchored eyes positioned ^ ^ ^ run function weapon:origin/short_sword
    execute anchored eyes positioned ^0.25 ^ ^ run function weapon:origin/short_sword
    execute anchored eyes positioned ^-0.25 ^ ^ run function weapon:origin/short_sword
...
##属性分岐-計算＆攻撃
    #無属性
    execute as @s[scores={stat_player.Attribute=0}] at @s run function weapon:item/8_rod/assets/none
